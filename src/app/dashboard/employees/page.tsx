...
import { listEmployees, listEmployeeRoles, expiringDocuments } from "@/features/workforce/queries";
...
export default async function EmployeesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ rows, total, page, size }, roles, expiring] = await Promise.all([listEmployees(ctx, sp), listEmployeeRoles(ctx), expiringDocuments(ctx, 30)]);
    ...
    <>
      ...
      <DataTable rows={rows} rowKey={(r) => r.id} rowHref={(r) => `/dashboard/employees/${r.id}`}
        columns={[
          ...
          { key: "p", header: "Portal", render: (r) => (r.user_id ? "Linked" : <span className="text-muted-light">—</span>) },
        ]}
