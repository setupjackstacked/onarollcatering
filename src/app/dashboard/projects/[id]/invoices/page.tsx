...
export default async function InvoicesTab({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { rows } = await listInvoices(ctx, await searchParams, { projectId: id });
  return (
    <div className="space-y-4">
      {ctx.can("finance.write") ? <div><ActionLink href={`/dashboard/invoices/new?project=${id}`} variant="copper">New invoice</ActionLink></div> : null}
