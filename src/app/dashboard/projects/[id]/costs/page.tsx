  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-6">
        <Metric label="Revenue (contract)" value={formatMoney(revenue, { showPence: false })} hint={`Invoiced ${formatMoney(toPence(fin?.invoiced_net), { showPence: false })}`} />
        <Metric label="Estimated cost" value={formatMoney(est, { showPence: false })} />
        <Metric label="Committed" value={formatMoney(committed, { showPence: false })} />
        <Metric label="Actual" value={formatMoney(actual, { showPence: false })} />
        <Metric label="Gross profit (forecast)" value={formatMoney(revenue - committed - actual, { showPence: false })} tone={revenue - committed - actual < revenue - est ? "warning" : "default"} hint={`Estimated ${formatMoney(revenue - est, { showPence: false })}`} />
              ...
              {breakdown.filter((b) => Number(b.estimated) || Number(b.committed) || Number(b.actual)).map((b) => {
                ...
                return (
                  <tr key={b.category}>
                    ...
                    <td className="py-2 pr-3 text-right num-lining">{formatMoney(e)}</td>
                    <td className="py-2 pr-3 text-right num-lining">{formatMoney(toPence(b.committed))}</td>
                    <td className="py-2 pr-3 text-right num-lining">{formatMoney(toPence(b.actual))}</td>
                    <td className={`py-2 text-right num-lining ${v < 0 ? "text-status-danger" : "text-status-success"}`}>{v < 0 ? "−" : ""}{formatMoney(Math.abs(v))}</td>
