...
export default async function QuotesTab({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const { rows } = await listQuotes(ctx, await searchParams, { projectId: id });
  return (
    <div className="space-y-4">
      {ctx.can("sales.write") ? <div><ActionLink href={`/dashboard/quotes/new?project=${id}`} variant="copper">New quote</ActionLink></div> : null}
